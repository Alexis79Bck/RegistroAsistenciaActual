...
<div class="row mb-2">
  ...
  <div class="col">
    ...
    <div class=" form-group">
      <div class="input-group input-group-sm mb-1 " id="grupoFechaInicio">
        ...
        {!! Form::date('fechaInicio', date('Y-m-d'), ['class'=>'form-control ', 'id' => 'ctrlFechaInicio', 'min'=>
        date('Y-m-d', strtotime($minFijadoFechaInicio)), 'max'=>date('Y-m-d', strtotime('today'))]) !!}
  ...
  <div class="col">
    <div class=" form-group">
      <div class="input-group input-group-sm mb-1 " id="grupoFechaFin">
        ...
        {!! Form::date('fechaFin', date('Y-m-d'), ['class'=>'form-control', 'id' => 'ctrlFechaFin', 'min'=>date('Y-m-d',
        strtotime($minFijadoFechaInicio)), 'max'=>date('Y-m-d', strtotime('today'))]) !!}
      </div>
    </div>
  </div>
  <div class="col">
  ...
  <div class="row  mb-2">
    ...
    <div class="col">
      <div class="form-check">
        {!! Form::radio('consultaPor', 'departamento', false,
        ['id'=>'consultaPorDepartamento','class'=>'form-check-input']) !!}
    ...
    <div class="col">
      ...
      {!! Form::text('cedula', null, ['id'=>'cedula', 'class'=>'form-control form-control-sm', 'onKeyPress'=>'return
      soloNumeros(event)','maxlength'=>'9']) !!}
    </div>
    {{-- @endif --}}

    @if (session()->get('rol_usuario') == 'Gerente' || session()->get('rol_usuario') == 'Supervisor')
    {!! Form::hidden('departamento', $idDepartamento) !!}
    @endif
  </div>
...
<div class="row  justify-content-md-center">
  ...
  <div class="col-md-auto">
    ...
    <a class="btn btn-outline-danger" href="{{ route ('impReporte')}}" target="_blank" type="button"
      form="formConsultar"><i class="fs-4 far fa-file-pdf fa-lg"></i></i></a>
...
<div class="row justify-content-center  mb-4" id="resultadoBusqueda">
  ...
  @case(1)
  @if ($ctData > 0)
  @for ($i = 0; $i < $ctDepartamento; $i++) @if ($ctReg[$i]==1) <table class=" shadow table table-bordered border-dark">
    ...
    @for ($j = 0; $j < $ctData; $j++) @if ($departamento[$i]->nombre == $info[$i][$j]['departamento'])
      @for ($k = 0; $k <= $ctFecha; $k++) @if ($lstFecha[$k]==$info[$i][$j]['fecha']) <tr>
        ...
        <td data-label="Hora Entrada">
          ...
          <span class="fw-bold h4 fs-6 text-danger"> {{$info[$i][$j]['horaEnt']}}</span>
          @else
          {{$info[$i][$j]['horaEnt']}}
          @endif
        ...
        <td data-label="Hora Salida">
          ...
          @else
          @if ($info[$i][$j]['Nocturno'])
          <span class="fw-bold h4 fs-6 text-danger">({{$info[$i][$j]['fechaSal']}}) {{$info[$i][$j]['horaSal']}}</span>
          @else
          {{$info[$i][$j]['horaSal']}}
          @endif
        ...
        @case(2)
        @if ($ctData > 0)
        @for ($i = 0; $i < $ctDepartamento; $i++) <table class=" shadow table table-bordered border-dark ">
          <tr class="bg-primary text-light  fw-bold text-center">
          ...
          @for ($j = 0; $j < $ctData; $j++) @if ($departamento[$i]->nombre == $info[$i][$j]['departamento'])
            <tr>
              ...
              <td data-label="Hora Salida">
                ...
                @else
                @if ($info[$i][$j]['Nocturno'])
                <span class="fw-bold h4 fs-6 text-danger">({{$info[$i][$j]['fechaSal']}})
                  {{$info[$i][$j]['horaSal']}}</span>
                @else
                {{$info[$i][$j]['horaSal']}}
                @endif
                @endif
            ...
            <table class=" shadow table table-bordered border-dark ">
              ...
              @else
              @for ($j = 0; $j < $ctData; $j++) @for ($k=0; $k <=$ctFecha; $k++) @if ($lstFecha[$k]==$info[$j]['fecha'])
                <tr>
                ...
                <td data-label="Hora Entrada">
                  ...
                  @else
                  @if ($info[$j]['Nocturno'])
                  <span class="fw-bold h4 fs-6 text-danger"> {{$info[$j]['horaEnt']}}</span>
                  @else
                  {{$info[$j]['horaEnt']}}
                  @endif
                  @endif
                ...
                <td data-label="Hora Salida">
                  ...
                  @else
                  @if ($info[$j]['Nocturno'])
                  <span class="fw-bold h4 fs-6 text-danger">({{$info[$j]['fechaSal']}}) {{$info[$j]['horaSal']}}</span>
                  @else
                  {{$info[$j]['horaSal']}}
                  @endif
                  @endif
            ...
            <table class=" shadow table table-bordered border-dark ">
              ...
              @for ($j = 0; $j < $ctData; $j++) <tr>
                ...
                <td data-label="Hora Salida">
                  ...
                  @else
                  @if ($info[$i][$j]['nocturno'])
                  <span class="fw-bold fs-4 text-danger">({{$info[$i][$j]['fechaSal']}})
                    {{$info[$i][$j]['horaSal']}}</span>
                  @else
                  {{$info[$i][$j]['horaSal']}}
                  @endif
        ...
        if (this.checked) {
          ...
          $.get('departamentos', function(resp){
            ...
            for (i = 0; i < resp.length; i++) {
              ...
              console.log(resp[i].codigo + ' ' + resp[i].nombre)
            }
          });
